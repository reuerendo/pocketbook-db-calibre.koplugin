...
local function slim(book, is_search)
    ...
    for _, k in ipairs(is_search and search_used_metadata or used_metadata) do
        ...
        elseif k == "_is_read_" then
            if book[k] ~= nil then
                slim_book[k] = book[k]
            end
        elseif k == "_last_read_date_" then
            if book[k] ~= nil then
                slim_book[k] = book[k]
            end
        elseif k == "_format_mtime_" then
            if book[k] ~= nil then
                slim_book[k] = book[k]
            end
        elseif k == "user_metadata" then
            if book[k] ~= nil then
                slim_book[k] = book[k]
            end
        else
...
function CalibreMetadata:updateBook(book)
    ...
    if index then
        local existing_book = self.books[index]
        local updated_book = slim(book)
        
        if existing_book["_is_read_"] ~= nil and updated_book["_is_read_"] == nil then
            updated_book["_is_read_"] = existing_book["_is_read_"]
        end
        if existing_book["_last_read_date_"] ~= nil and updated_book["_last_read_date_"] == nil then
            updated_book["_last_read_date_"] = existing_book["_last_read_date_"]
        end
        if existing_book["_format_mtime_"] ~= nil and updated_book["_format_mtime_"] == nil then
            updated_book["_format_mtime_"] = existing_book["_format_mtime_"]
        end
        if existing_book["user_metadata"] ~= nil and updated_book["user_metadata"] == nil then
            updated_book["user_metadata"] = existing_book["user_metadata"]
        end
...
function CalibreMetadata:updateBookSyncData(lpath, sync_data)
    local _, index = self:getBookUuid(lpath)
    if index then
        local book = self.books[index]
        
        if sync_data._is_read_ ~= nil then
            book["_is_read_"] = sync_data._is_read_
        end
        if sync_data._last_read_date_ ~= nil then
            book["_last_read_date_"] = sync_data._last_read_date_
        end
        if sync_data._format_mtime_ ~= nil then
            book["_format_mtime_"] = sync_data._format_mtime_
        end
        if sync_data.user_metadata ~= nil then
            book["user_metadata"] = sync_data.user_metadata
        end
